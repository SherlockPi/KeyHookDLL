...
namespace LOGGER
{
	...
	string CLogger::GetTime()
	{
		...
		//2018-10-31 11:01:01 670
		timeb   tp;
		ftime(&tp);
		string strTime = FormatString("%04d-%02d-%02d %02d:%02d:%02d %04d ", tm1.tm_year + 1900, tm1.tm_mon + 1, tm1.tm_mday, tm1.tm_hour, tm1.tm_min, tm1.tm_sec, tp.millitm);
	...
	{
		//盘符(包括冒号：),路径(包含前面&后面的\，不含盘符&文件名),文件名(不含扩展名),扩展名(包含前面的.)
		char szFilePath[MAX_PATH] = { 0 }, szDrive[MAX_PATH] = { 0 }, szDir[MAX_PATH] = { 0 }, szFileName[MAX_PATH] = { 0 }, szExt[MAX_PATH] = { 0 };

		GetModuleFileNameA(NULL, szFilePath, sizeof(szFilePath));		//获取当前工程直到.exe的绝对路径的字符串 如果没找到就返回0

		_splitpath_s(szFilePath, szDrive, szDir, szFileName, szExt);	//绝对路径分割为盘符、路径、文件名、扩展名。
